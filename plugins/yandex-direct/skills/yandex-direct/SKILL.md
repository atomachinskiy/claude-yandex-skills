...
Как устроен доступ, почему `Bearer` и не `OAuth`, чем обычный кабинет отличается от агентского, что означают коды 53, 58, 8000 и 8800, сколько стоят запросы в баллах: `references/ОСНОВЫ.md`. Читать до того, как чинить ошибку наугад.

## Корпоративное приложение и агентский аккаунт

Два необязательных параметра, оба читаются из окружения или из `config/.env`:

- `YANDEX_DIRECT_CLIENT_ID` — client_id приложения, у которого **уже одобрена заявка на доступ к API** в кабинете Директа. По умолчанию берётся «Claud direct». Задавать, когда у компании есть своё одобренное приложение: новую заявку подавать не нужно, достаточно авторизоваться под ним.
- `YANDEX_DIRECT_CLIENT_LOGIN` — логин клиента в Директе. Нужен, когда токен выпущен под **агентским** аккаунтом: агентский токен сам по себе не знает, о чьих кампаниях спрашивают, и API отвечает пусто или ошибкой. Для прямого рекламодателя не задавать.

```bash
export YANDEX_DIRECT_CLIENT_ID=29e4a3c5b3024dd79697590ee3084ac4
export YANDEX_DIRECT_APP_NAME="Приложение для сбора показателей"
export YANDEX_DIRECT_CLIENT_LOGIN=kontur.online
```
...
Вне скилла: комбинаторные объявления целиком, фиды и товарные кампании, загрузка изображений, превью для согласования, аудит автоконвертации. Если задача про это, скажи человеку, что скилл такого не делает, и не собирай запрос по догадке.
