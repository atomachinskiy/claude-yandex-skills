...
товарной рекламы и изменении отбора товаров. Сначала определите, что меняется:
...
механику ЕПК. `shopping.py` работает с `ShoppingAd`; отдельные объявления
каталога `ListingAd` читает `ads.py`, но эта команда записи их не меняет.
...
создания и [управления бюджетом](BUDGETS.md).
