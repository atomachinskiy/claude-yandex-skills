...
description: |
  Анализ поискового спроса (Wordstat). ⚠️ Доступ к scope требует ручной заявки в поддержку Яндекса.
  Использует общий OAuth-токен Яндекса (плагин yandex-auth). Cache-first,
  лимит stdout 30 строк по умолчанию.
  Triggers: yandex-wordstat, wordstat, яндекс wordstat.
---
...
Scope в OAuth-приложении: `direct:api-stats`.
...
База: `https://api.direct.yandex.ru/v4/json`
